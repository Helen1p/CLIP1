# 需要save visual encoder features of different layers时把model1.py复制到model.py
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(len(self.resblocks)):
            ...
            feature.append(x.clone().permute(1, 0, 2))
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, text):
        # DDP时需要with aotocast():
        image_features, feature = self.encode_image(image) # [bs, 512]
        text_features = self.encode_text(text) # [bs, 512]

        # normalized features
        image_features = image_features / image_features.norm(dim=1, keepdim=True) # [bs, 512]
        text_features = text_features / text_features.norm(dim=1, keepdim=True) # [bs, 512]
        ...
        return logits_per_image, logits_per_text, feature
...
if __name__=='__main__':
    checkpoint=torch.load(r'/data/pxg1/data/checkpoint_epoch1.pth')
