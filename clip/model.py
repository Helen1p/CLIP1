...
def get_optim_params(model_name: str):
    if model_name in ['ViT-B/32', 'ViT-B/16']:
        ...
        return ['visual.layer4.2.conv1.weight',
                ...
                'visual.layer4.2.bn1.bias',
                'visual.layer4.2.bn1.running_mean',
                'visual.layer4.2.bn1.running_var',
                'visual.layer4.2.bn1.num_batches_tracked',
                'visual.layer4.2.conv2.weight',
                'visual.layer4.2.bn2.weight',
                'visual.layer4.2.bn2.bias',
                'visual.layer4.2.bn2.running_mean',
                'visual.layer4.2.bn2.running_var',
                'visual.layer4.2.bn2.num_batches_tracked',
                'visual.layer4.2.conv3.weight',
                'visual.layer4.2.bn3.weight',
                'visual.layer4.2.bn3.bias',
                'visual.layer4.2.bn3.running_mean',
                'visual.layer4.2.bn3.running_var',
                'visual.layer4.2.bn3.num_batches_tracked',
...
class ensemble_fusion(nn.Module):
    ...
    def forward(self, image, text):
        all=torch.cat([image, text], dim=-1)
        all=self.fc(all)+image
        return all
...
class CustomCLIP(nn.Module):
    def __init__(self,
        ...
        self.load_from_clip = load_from_clip
    ...
    def forward(self, image, text, prior: Optional[torch.Tensor] = None):
        ...
        #     text1 = text1 / text1.norm(dim=1, keepdim=True) # [bs, 768]
        #     image_features = self.fuse(image_features, text1)
...
def build_model(state_dict: dict, mode: str, frozen_layers: bool, load_from_clip=True, name: Optional[str] = None):
    ...
    if mode=='fine_tune':
        ...
        if not load_from_clip:
            ...
            keep_len = 20
            posisitonal_embedding_new = torch.zeros([4*length-3*keep_len, dim], dtype=model.dtype) #248
...
if __name__=='__main__':
    checkpoint=torch.load(r'/data/pxg1/data/checkpoint/checkpoint_epoch4.pth')
    # checkpoint=torch.load(r'/data/pxg1/data/checkpoint/longclip-B.pt')
    # print(checkpoint.keys())
    # print(checkpoint['optimizer'].keys())
    # checkpoint1=torch.load(r'/data/pxg1/data/checkpoint_ft_all/checkpoint_epoch4.pth')

    print(checkpoint['state_dict']["visual.conv1.weight"].shape[-1])
    # ["positional_embedding"].shape
    # build_model(checkpoint['state_dict'], mode='test', frozen_layers=True)